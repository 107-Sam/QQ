...
let Account = ["【Sam】"];
  ...
  if (process.env.YOUTH_DAILY_URL && process.env.YOUTH_DAILY_URL.indexOf('\n') > -1) {
  URL = process.env.YOUTH_DAILY_URL.split('\n');
  ...
  if (process.env.YOUTH_DAILY_BD && process.env.YOUTH_DAILY_BD.indexOf('\n') > -1) {
  BD = process.env.YOUTH_DAILY_BD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } 
  else
  {
  ...
  if (process.env.YOUTH_DAILYREWARD_BD && process.env.YOUTH_DAILYREWARD_BD.indexOf('\n') > -1) {
  TimeBD = process.env.YOUTH_DAILYREWARD_BD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } 
  else
  {
  ...
  if (process.env.YOUTH_DAILY_HD && process.env.YOUTH_DAILY_HD.indexOf('\n') > -1) {
  HD = process.env.YOUTH_DAILY_HD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } 
  else
  {
       ...
       let myrequest = {
        ...
        $.post(myrequest, async(error, response, data) => {
          try{
           let readres = JSON.parse(data);
            //console.log(readres)
           if (readres.error_code == '0') {
            console.log(`【share】${readres.message}；`);
            //detail += `【share】${readres.message}；\n`;
            }
           else  {
            console.log(`【share】${readres.message}；`);
            //detail += `【share】${readres.message}；\n`;
